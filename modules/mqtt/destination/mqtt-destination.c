...
_format_stats_instance(LogThreadedDestDriver *d)
{
  ...
  if (((LogPipe *)d)->persist_name)
    ...
    g_snprintf(stats_instance, sizeof(stats_instance), "mqtt,destination,%s,%s", self->option.address,
...
_format_persist_name(const LogPipe *d)
{
  ...
  if (d->persist_name)
    ...
    g_snprintf(persist_name, sizeof(persist_name), "mqtt-destination.(%s,%s)", self->option.address,
               self->topic_name->template);
...
_init(LogPipe *d)
{
  ...
  if (self->option.http_proxy)
    {
      ...
      g_free(self->option.http_proxy);
      self->option.http_proxy = NULL;
    }
#endif
  ...
  if (_topic_name_is_a_template(self) && self->option.fallback_topic_name == NULL)
    {
  ...
  if (self->option.client_id == NULL)
    mqtt_option_set_client_id(&self->option, _format_persist_name(d));
