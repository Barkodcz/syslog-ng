...
_mqtt_send(LogThreadedDestWorker *s, gchar *msg, const gchar *topic)
{
  ...
  pubmsg.qos = mqtt_client_options_get_qos(&owner->option);
  pubmsg.retained = 0;
...
_connect(LogThreadedDestWorker *s)
{
  ...
  mqtt_client_options_to_mqtt_connection_option(&owner->option, &conn_opts);
  ...
  if ((rc = MQTTClient_connect(self->client, &conn_opts)) != MQTTCLIENT_SUCCESS)
    {
      msg_error("Error connecting mqtt client",
                ...
                evt_tag_str("client_id", mqtt_client_options_get_client_id(&owner->option)),
                log_pipe_location_tag(&owner->super.super.super.super));
      return FALSE;
    }
...
_thread_init(LogThreadedDestWorker *s)
{
  ...
  if ((rc = MQTTClient_create(&self->client, mqtt_client_options_get_address(&owner->option),
                              mqtt_client_options_get_client_id(&owner->option),
                              MQTTCLIENT_PERSISTENCE_NONE, NULL)) != MQTTCLIENT_SUCCESS)
    {
      msg_error("Error creating mqtt client",
                evt_tag_str("address", mqtt_client_options_get_address(&owner->option)),
                evt_tag_str("error code", MQTTClient_strerror(rc)),
                evt_tag_str("client_id", mqtt_client_options_get_client_id(&owner->option)),
                log_pipe_location_tag(&owner->super.super.super.super));
      return FALSE;
...
mqtt_dw_new(LogThreadedDestDriver *o, gint worker_index)
{
  ...
  mqtt_client_options_set_log_error_fn(&owner->option, _log_ssl_errors);
