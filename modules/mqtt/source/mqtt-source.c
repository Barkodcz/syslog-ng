...
_format_persist_name(const LogPipe *s)
{
  ...
  if (((LogPipe *)s)->persist_name)
    ...
    g_snprintf(stats_instance, sizeof(stats_instance), "mqtt,source,%s,%s", mqtt_client_options_get_address(&self->option),
               self->topic);
...
_format_stats_instance(LogThreadedSourceDriver *s)
{
  ...
  if (p->persist_name)
    ...
    g_snprintf(persist_name, sizeof(persist_name), "mqtt-source.(%s,%s)", mqtt_client_options_get_address(&self->option),
               self->topic);
...
_subscribe_topic(MQTTSourceDriver *self)
{
  ...
  if ((rc = MQTTClient_subscribe(self->client, self->topic,
                                 mqtt_client_options_get_qos(&self->option))) != MQTTCLIENT_SUCCESS)
    {
      msg_error("Error while setting callbacks",
                evt_tag_str("topic", self->topic),
                evt_tag_int("qos", mqtt_client_options_get_qos(&self->option)),
                evt_tag_str("error code", MQTTClient_strerror(rc)),
...
_connect(LogThreadedFetcherDriver *s)
{
  ...
  mqtt_client_options_to_mqtt_connection_option(&self->option, &conn_opts);
...
{
  gint rc;
  if ((rc = MQTTClient_unsubscribe(self->client, self->topic)) != MQTTCLIENT_SUCCESS)
    {
      return FALSE;
    }
...
mqtt_sd_new(GlobalConfig *cfg)
{
  ...
  mqtt_client_options_set_log_error_fn(&self->option, _log_ssl_errors);
